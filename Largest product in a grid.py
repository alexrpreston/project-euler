...
for i in range(0,20): #Rows
    for j in range(0,20): #Cols
        if i + 3 <= 19: #Up and Down
            currProd = grid[i][j] * grid[i+1][j] * grid[i+2][j] * grid[i+3][j]
            print("Up/Down", end="")
            print(grid[i][j], grid[i+1][j], grid[i+2][j], grid[i+3][j], currProd)
            largestProduct = max(currProd,largestProduct)
            results.append(currProd)
        if j + 3 <= 19: #Left and Right
            currProd = grid[i][j] * grid[i][j+1] * grid[i][j+2] * grid[i][j+3]
            print("L/R", end="")
            print(grid[i][j], grid[i][j+1], grid[i][j+2], grid[i][j+3], currProd)
            largestProduct = max(currProd,largestProduct)
            results.append(currProd)
        if j + 3 <= 19 and i + 3 <= 19:
            currProd = grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3]
            print("Diag down", end="")
            print(grid[i][j], grid[i+1][j+1], grid[i+2][j+2], grid[i+3][j+3], currProd)
            largestProduct = max(currProd,largestProduct)
            results.append(currProd)
        if j + 3 <= 19 and i - 3 >= 0:
            currProd = grid[i][j] * grid[i-1][j+1] * grid[i-2][j+2] * grid[i-3][j+3]
